...
5. **User-specified subset**: If the user's instructions mention specific files or packages to include, filter to only those. If unspecified and there are many unrelated changes, ask whether to include everything.
...
If already on a non-main branch with relevant commits, offer to reuse it.
...
Show proposed name and allow the user to override, then create:
```bash
...
**`## Test Plan`** — Checklist of what was tested or needs testing. Helps reviewers know what to verify.
...
## Step 11: Present Plan, Confirm, Push, and Create PR

### Present the full plan

```
Branch:      feat/chromadb-quantization-support
Commits:     1) feat(chromadb): add quantization support (8 files)
PR Title:    feat(chromadb): add quantization support
Labels:      p:chromadb, t:feature
Assignee:    davidmigloz
Draft:       no

--- PR Description ---
## Summary
- Add quantization support for collections...
...
The user may edit any part. If they abort, do not push or create anything.
...
10. **Mixed staged and unstaged changes** → show both clearly, let user decide what to include
