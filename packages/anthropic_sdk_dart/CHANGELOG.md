...
Completes the four-part Anthropic spec refresh and layers on several beta capabilities: [prompt-cache diagnostics](https://platform.claude.com/docs/en/build-with-claude/cache-diagnostics) (pass `DiagnosticsParam.previousMessageId` and read `Message.diagnostics?.cacheMissReason` to learn *why* a cache prefix diverged), full request- and response-side [search-result citations](https://docs.anthropic.com/en/docs/build-with-claude/search-results) for grounding answers in your own RAG content, multiagent orchestration with outcome evaluations and rubrics, Session Threads (CRUD + streaming), inbound webhook event parsing, and MCP-OAuth credential validation. Also tolerates missing/null `signature` on streamed `ThinkingBlock`s from Anthropic-compatible servers (e.g. MiniMax). **Breaking:** the surface is small and mostly forward-compatibility hardening — new variants were added to the exported sealed unions `Citation`, `InputContentBlock`, `SessionEvent`, and `EventParams`, so only code with exhaustive `switch` statements that lack a wildcard/`Unknown*` branch needs updating; additionally `UserProfile.relationship` is now a required constructor parameter. Pure-deserialization consumers are unaffected.

- **BREAKING** **FEAT**: Cache diagnostics, session & skill updates ([#236](https://github.com/davidmigloz/ai_clients_dart/issues/236)). ([6d1d488f](https://github.com/davidmigloz/ai_clients_dart/commit/6d1d488f7d0ff3f768dc21da4342dc7cff81694c))
- **BREAKING** **FEAT**: Request-side search_result blocks & per-citation locations ([#235](https://github.com/davidmigloz/ai_clients_dart/issues/235)). ([1f10a79f](https://github.com/davidmigloz/ai_clients_dart/commit/1f10a79fbdfc8c267e3125413cb265b9e7dce540))
- **BREAKING** **FEAT**: Support search_result_location citations and add Unknown fallback ([#233](https://github.com/davidmigloz/ai_clients_dart/issues/233)). ([68e0f797](https://github.com/davidmigloz/ai_clients_dart/commit/68e0f7972ca93d40038f4969b27a0f05fa3e66dd))
- **BREAKING** **FEAT**: Add multiagent orchestration, outcome evaluations & rubrics ([#230](https://github.com/davidmigloz/ai_clients_dart/issues/230)). ([d7ac4376](https://github.com/davidmigloz/ai_clients_dart/commit/d7ac4376119b0e3fd2f3b3139ba7ce371e6198b6))
- **BREAKING** **FEAT**: Spec refresh, relationship + thread IDs ([#223](https://github.com/davidmigloz/ai_clients_dart/issues/223)). ([3c6a4bdc](https://github.com/davidmigloz/ai_clients_dart/commit/3c6a4bdc85d59b6a076e9141bee7ff839cc570bd))
- **FEAT**: Add webhook events & credential validation ([#231](https://github.com/davidmigloz/ai_clients_dart/issues/231)). ([075005a6](https://github.com/davidmigloz/ai_clients_dart/commit/075005a6a0d0a18abb9c5eba38b49406441d0213))
- **FEAT**: Add Session Threads (CRUD + streaming) ([#224](https://github.com/davidmigloz/ai_clients_dart/issues/224)). ([5367e35b](https://github.com/davidmigloz/ai_clients_dart/commit/5367e35bf9f95ee74981130325d4b4cedbdf8109))
- **FIX**: Tolerate missing/null signature on ThinkingBlock.fromJson ([#226](https://github.com/davidmigloz/ai_clients_dart/issues/226)). ([9e17743a](https://github.com/davidmigloz/ai_clients_dart/commit/9e17743a6293e51e165aee4863d77f4998e6c72d))
