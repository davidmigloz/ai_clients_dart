...
> [!CAUTION]
> This release has breaking changes to the experimental Interactions API.

Refactors the Interactions API to match the [Interactions API breaking changes (May 2026)](https://ai.google.dev/gemini-api/docs/interactions-breaking-changes-may-2026), which move tool calls and tool results out of `Content` into a new `Step` sealed family, restructure the SSE event stream, add the Webhooks resource, and introduce a `ResponseFormat` sealed family. Because the Interactions API is marked experimental in the spec, the migration is a hard rename with no compatibility shims. The client opts into the new schema by sending the `Api-Revision: 2026-05-20` request header on every interactions request (matching the official `google-genai` SDK); the new schema becomes the API default on 2026-05-26 and the legacy `outputs` schema is removed on 2026-06-08.

- **BREAKING**: `Interaction.outputs` is removed and replaced by `Interaction.steps` (typed as `List<InteractionStep>`). The same rename applies to `CreateModelInteractionParams.steps` and `CreateAgentInteractionParams.steps`.
- **BREAKING**: Tool-call/result content classes are removed: `FunctionCallContent`, `FunctionResultContent`, `CodeExecutionCall/ResultContent`, `FileSearchCall/ResultContent`, `GoogleSearch/MapsCall/ResultContent`, `McpServerToolCall/ResultContent`, `UrlContextCall/ResultContent`, and `ThoughtContent`. They are replaced by 17 `InteractionStep` variants (`FunctionCallStep`, `FunctionResultStep`, `ThoughtStep`, `ModelOutputStep`, `UserInputStep`, etc.).
- **BREAKING**: Stream event renames: `InteractionStartEvent` → `InteractionCreatedEvent`, `InteractionCompleteEvent` → `InteractionCompletedEvent`, `ContentDeltaEvent`/`ContentStartEvent`/`ContentStopEvent` → `StepDeltaEvent`/`StepStartEvent`/`StepStopEvent`. `StepStartEvent.step` is now an `InteractionStep` rather than a content block.
- **BREAKING**: Sealed `ContentDeltaData` is renamed to `StepDeltaData`. The 14 tool-related delta variants (`FunctionCallDelta`, `CodeExecutionCallDelta`, etc.) are removed. A new `ArgumentsDelta` variant carries partial JSON-encoded tool-call arguments. The `TextAnnotationDelta` discriminator is corrected to `text_annotation_delta` (was `text_annotation`).
- **BREAKING**: `TurnsInput`, `Turn`, and the `TurnContent` family are removed. The spec's `InteractionsInput` union no longer includes turns (it is now `String | List<InteractionStep> | List<InteractionContent> | InteractionContent`), and the API rejects `turn_list` under the new schema. Use `StepListInput` (with `UserInputStep`/`ModelOutputStep`) to provide conversation history inline.
- **BREAKING**: `AudioContent.rate` is renamed to `AudioContent.sampleRate` (JSON `sample_rate`). The same rename applies to `AudioDelta.rate` → `AudioDelta.sampleRate`.
- **BREAKING**: `FileSearchResult.customMetadata` is removed.
- **BREAKING**: `ToolResult.contentList` element type changes from `List<InteractionContent>` to `List<FunctionResultSubcontent>` to match the new spec's union (only `TextContent`/`ImageContent` subcontent is accepted).
- **BREAKING**: `Interaction.functionCallOutputs`/`thoughtOutputs` extension getters are renamed to `functionCallSteps`/`thoughtSteps`. `googleMapsCallOutputs`/`googleMapsResultOutputs` → `googleMapsCallSteps`/`googleMapsResultSteps`. `imageOutputs`/`audioOutputs`/`textOutputs` now project across `ModelOutputStep.content`.
- **FEAT**: New `Webhook` resource (`client.webhooks`) with create/list/get/update/delete, `:ping`, and `:rotateSigningSecret` endpoints. Includes typed models (`Webhook`, `WebhookUpdate`, `WebhookState`, `SigningSecret`, `ListWebhooksResponse`, `RotateSigningSecretRequest/Response`) and a hand-written `WebhookEvent` sealed family for delivery payloads (`batch.succeeded`/`expired`/`failed`, `interaction.requires_action`/`completed`/`failed`, `video.generated`).
- **FEAT**: New `WebhookConfig` field on `Interaction`/`CreateModelInteractionParams`/`CreateAgentInteractionParams` for overriding the registered webhooks on a per-interaction basis.
- **FEAT**: New `ResponseFormat` sealed family (`AudioResponseFormat`, `TextResponseFormat`, `ImageResponseFormat`, `VideoResponseFormat`, plus an `UnknownResponseFormat` catch-all for the spec's open `{ "type": object }` member) with enum-typed `mimeType`/`aspectRatio`/`delivery`/`imageSize`. The family is wired onto `Interaction`, `CreateModelInteractionParams`, and `CreateAgentInteractionParams` (and the `create`/`createWithAgent`/`createStream` resource methods) via a new `ResponseFormatConfig` sealed wrapper that models the spec's `response_format` union as either a single format (`SingleResponseFormat`) or a list (`ResponseFormatList`). This enables structured JSON output (`TextResponseFormat` with `mimeType: application/json` + `schema`). Note: the response-format fields serialize with snake_case wire keys (`mime_type`, `bit_rate`, `sample_rate`, `aspect_ratio`, `image_size`) to match the live API and the official SDK; the spec's camelCase property names are not used on the wire.
- **FEAT**: `Usage.groundingToolCount` field exposes per-tool grounding counts (`google_search`, `google_maps`, `retrieval`).
- **FEAT**: `FileCitation` gains `customMetadata`, `mediaId`, and `pageNumber` fields.
- **FEAT**: New `StepListInput` variant on `InteractionInput` for passing function/tool result steps back into a paused interaction, or for providing conversation history inline.
- **FEAT**: Streaming `step.start` events carry a partial step skeleton (the `arguments`/`result` payload streams in via subsequent `step.delta` events); these fields now default to empty rather than failing to parse. Tool-call/result `step.delta` payloads that are not part of the published `StepDeltaData` schema (e.g. `google_search_call`, `url_context_result`) are surfaced as a new `UnknownStepDelta` fallback variant (raw JSON preserved) instead of throwing, keeping streams resilient and forward-compatible.
