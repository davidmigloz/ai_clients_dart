...
class RequestBuilder {
  ...
  ///
  /// The [path] is a URL path (e.g. `/v1/chat/completions`); pass query
  /// parameters via [queryParams] rather than embedding them in [path].
  /// Values may be a `String` or an `Iterable<String>` (rendered as repeated
  /// keys, e.g. `?status=a&status=b`).
  ///
  ...
  Uri buildUrl(String path, {Map<String, dynamic>? queryParams}) {
    ...
    // correctly (e.g. a proxy sub-path or an `?api-version=` param).
